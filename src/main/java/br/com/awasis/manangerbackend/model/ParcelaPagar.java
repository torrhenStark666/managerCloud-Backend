...
@Data
@Entity
@Table(name="tb_parcela_pagar")
@SQLDelete(sql = "update tb_parcela_pagar set excluido = true where id_parcela_pagar = ?")
@Where(clause = "excluido = false")
public class ParcelaPagar {

    @ManyToMany(mappedBy = "parcelasPagar")
    @JsonIgnore
    ...
    @Column(name="valor_original" ,scale = 2, precision = 10)  
    private double valorOriginal;
    
    @Column(name="valor_pagar",scale = 2, precision = 10)
    ...
    @Column(name="valor_juro",scale = 2, precision = 10)
