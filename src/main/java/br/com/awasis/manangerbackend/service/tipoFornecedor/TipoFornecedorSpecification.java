 ...
 * @author alecsander
 */
public class TipoFornecedorSpecification {
    
    public static Specification byIdSpecification(long id){
        return (Root root, CriteriaQuery query, CriteriaBuilder builder) ->{
            Predicate equalPredicate = builder.equal(root.get(TipoFornecedor_.idTipoFornecedor), id);
    ...
    public static Specification byAtivoSpecification(boolean ativo){
        return (Root root, CriteriaQuery query, CriteriaBuilder builder) ->{
          Predicate equalPredicate = builder.equal(root.get(TipoFornecedor_.ativo), ativo);
          return equalPredicate;
        };
    }
    ...
    public static Specification byDescricaoSpecification(String descricao){
        return (Root root, CriteriaQuery query, CriteriaBuilder builder) ->{
            Predicate equalPredicate = builder.like(root.get(TipoFornecedor_.descricao), "%"+descricao.trim()+"%");
            return equalPredicate;
        };
    }
