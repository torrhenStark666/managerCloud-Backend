...
import br.com.awasis.manangerbackend.model.Empresa_;
...
public class EmpresaSpecification {
    ...
    public static Specification byIdSpecification(long id){
        return (Root root, CriteriaQuery query, CriteriaBuilder builder) ->{
            Predicate equalPredicate = builder.equal(root.get(Empresa_.idEmpresa), id);
            return equalPredicate;
        };
    }
    
    public static Specification byNomeFantasiaSpecification(String str){
        return (Root root, CriteriaQuery query, CriteriaBuilder builder) ->{
            Predicate p = builder.like(root.get(Empresa_.nomeFantasia), "%"+str+"%");
            return p;
        };
    }
