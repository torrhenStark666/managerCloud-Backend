...
import br.com.awasis.manangerbackend.repository.CorRepository;
...
public class CorServiceImpl implements CorService {
    ...
    public Optional findById(long id) {
        return repository.findOne(CorSpecification.byIdSpecification(id));
    }
    ...
    public List find(Cor cp) {
         ...
         if(!cp.getDescricao().isEmpty() || !cp.getDescricao().isBlank()){
             where = CorSpecification.addClausula(where, CorSpecification.byDescricaoSpecification(cp.getDescricao()));
         }
         where = CorSpecification.addClausula(where, CorSpecification.byAtivoSpecification(cp.isAtivo()));
    ...
    public Optional<Cor> update(Cor cp, long id) {
                return repository.findById(id)
                 .map((record) ->{
                     ...
                     Cor updated = repository.save(record);
                     return updated;
