...
import br.com.awasis.manangerbackend.service.movimentacaoBancaria.MovimentacaoBancariaService;
...
public class MovimentacaoBancariaController {
    ...
    @PostMapping
    public MovimentacaoBancaria save(@RequestBody MovimentacaoBancaria b){
        return service.save(b);
    }
    
    @PostMapping(path = {"/find"})
    public List findByProperties(@RequestBody MovimentacaoBancaria b){
        return service.find(b);
    }
    
    @PutMapping(path = "/{id}")
    public ResponseEntity update(@PathVariable long id, @RequestBody MovimentacaoBancaria b){
