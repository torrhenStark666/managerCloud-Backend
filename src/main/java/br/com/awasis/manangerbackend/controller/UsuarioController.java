...
public class UsuarioController {
    ...
    public List findByProperties(@RequestBody Usuario gf){
        return service.find(gf);
    }
    
    @PostMapping(path = {"/findLogin"})
    public ResponseEntity findByLogin(@RequestBody Usuario gf){
        return (ResponseEntity) service.findLogin(gf.getLogin())
                .map(record -> ResponseEntity.ok(record))
                .orElse(ResponseEntity.notFound().build());
    }
    
    @PostMapping
    public Usuario save(@RequestBody Usuario gf){
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity update(@PathVariable long id, @RequestBody Usuario gf){
